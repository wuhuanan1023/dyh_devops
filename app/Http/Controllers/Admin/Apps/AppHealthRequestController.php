...
/**
 * 应用健康监测
 * Class AppHealthRequestController
 * @package App\Http\Controllers\Admin\Apps
 */
class AppHealthRequestController extends BaseController
{
    ...
    {
        $status = [];
        foreach (AppHealthRequest::STATUS_MAP as $key => $value) {
    ...
    public function list(Request $request)
    {
        ...
        $app_health_request_table   = (new AppHealthRequest())->getTable();
        $app_table                  = (new Apps())->getTable();
        $platform_table             = (new Platform())->getTable();

        $query = AppHealthRequest::query()
            ->select([
                'ahr.*',
                'a.app_name',
        ...
        $list = $this->getPagingRows($query, function (LengthAwarePaginator $paginator) {
            $paginator->getCollection()->transform(function ($row) {
                return [
                    ...
                    'status'        => $row['status'],
                    'status_remark' => AppHealthRequest::STATUS_MAP[$row['status']] ?? '',
    ...
    public function setStatus(Request $request)
    {
        ...
        if (!isset(AppHealthRequest::STATUS_MAP[$status])) {
